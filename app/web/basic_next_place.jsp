...
<html>
    ...
    <head>
        ...
        <title>Basic Location Report Next Places</title>
    ...
    <body>
        ...
        <div class="container">  
            <div class="well-small">
                <h2>Top-k Next Places</h2><hr>
                <!--A form for user to key in k-value, location and date and time, then it will be process in next place servlet-->
                <form action="next_place.do" method="post" >
                    <p>Please select a K-Value:</p><select name="k" required> 
                    ...
                    <select name="origin" required>
                        <%
                            ArrayList<String> semantic_places = LocationLookupManager.retrieveAllSemanticPlaces();
                            for (String semantic_place : semantic_places) {
                                out.println("<option " + Utility.selectedCheck((String) request.getAttribute("origin"), semantic_place) + ">" + semantic_place + "</option>");
                            }
                        %>
                    </select>
                ...
                <%
                    // Retrieves map containing semantic places and their respective number of users 
                    ArrayList<NextPlacesResults> nextPlacesResultList = new ArrayList<NextPlacesResults>();
                    Integer totalUserCount = (Integer) request.getAttribute("totalUserCount");
                    Integer totalUserVisit = (Integer) request.getAttribute("totalUserVisit");
                    ArrayList<String> outputList = (ArrayList<String>) request.getAttribute("outputList");
                    HashMap<String, ArrayList<NextPlacesResults>> newResult = (HashMap<String, ArrayList<NextPlacesResults>>) request.getAttribute("nextPlacesResult");
                    ...
                    if (outputList != null && !outputList.isEmpty()) {
                        ...
                        if (newResult.size() > 0) {
                            System.out.println("SIZE OF NEXT PLACES RESULTS LIST: " + nextPlacesResultList.size());
                            out.println("<div class='well well-small text-center'> <b>Total users found: " + totalUserCount + "</b><br/><b>Total users who visited another place: " + totalUserVisit + "</b></div>");
                            out.println("<table class='table table-striped table-bordered'><tr><th width='25%'>Rank</th><th width='25%'>Semantic Place</th><th wide='25%'>Count</th><th width='25%'>Percentage over origin</th></tr>");
                            Iterator iter = newResult.keySet().iterator();
                            while (iter.hasNext()) {
                                nextPlacesResultList = newResult.get(iter.next());
                                NextPlacesResults n = nextPlacesResultList.get(0);
                                out.println("<tr><td rowspan = \" " + nextPlacesResultList.size() + "\">");
                                out.println(n.getRank() + "</td>");
                                out.println("<td>" + n.getSemantic_place() + "</td>");
                                out.println("<td  rowspan = \" " + nextPlacesResultList.size() + "\">" + n.getCount() + "</td>");
                                out.println("<td>" + Math.round((double) n.getCount() / totalUserCount * 100.0)  + "%</td>");
                                
                                
                                out.println("</tr>");
                                for (int i = 1; i < nextPlacesResultList.size(); i++) {
                                    NextPlacesResults result = nextPlacesResultList.get(i);
                                    out.println("<tr>");
